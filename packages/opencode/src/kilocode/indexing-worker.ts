...
async function handle(request: Request) {
  ...
  try {
    if (method === "dispose") {
      await dispose(request.key)
      send({ type: "result", id, method: "dispose", ok: true, value: undefined })
      return
    }
    ...
    if (method === "search") {
      ...
      send({ type: "result", id, method: "search", ok: true, value: value ?? [] })
      return
    }
    ...
    send({
      ...
      method,
      ok: false,
      error: `Unknown indexing worker method: ${method}`,
    })
  } catch (err) {
    const error = err instanceof Error ? err.message : String(err)
    send({ type: "result", id, method, ok: false, error })
  }
}
