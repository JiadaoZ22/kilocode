...
export function DialogIndexing(props: DialogIndexingProps) {
  ...
  const [overlay] = createResource(async () => {
    const timeout = new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error("Indexing config overlay load timed out")), 10000),
    )
    try {
      const response = await Promise.race([sdk.client.config.overlay({ scope: "project" }), timeout])
      return response.data
    } catch (err) {
      console.warn("Failed to load indexing config overlay:", err)
      return undefined
    }
  })
